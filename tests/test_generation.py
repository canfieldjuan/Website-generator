...
class RetiredRuntimeLauncherTests(unittest.TestCase):
    scripts = (
        Path(__file__).resolve().parents[1] / "scripts/start_llama_server.sh",
        Path(__file__).resolve().parents[1] / "scripts/start_vllm_server.sh",
    )

    def test_legacy_launchers_fail_closed_without_starting_a_runtime(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            marker = Path(temporary_directory) / "runtime-started"
            fake_vllm = Path(temporary_directory) / "vllm"
            fake_vllm.write_text(
                "#!/usr/bin/env bash\ntouch \"$RETIRED_RUNTIME_MARKER\"\n",
                encoding="utf-8",
            )
            ...
            environment = {
                **os.environ,
                ...
                "RETIRED_RUNTIME_MARKER": str(marker),
            }
            ...
            for script in self.scripts:
                for arguments in ((), ("--help",)):
                    with self.subTest(script=script.name, arguments=arguments):
                        ...
                        self.assertIn("operator-managed Ollama", completed.stderr)
                        self.assertIn(DEFAULT_LOCAL_MODEL, completed.stderr)
                        self.assertFalse(marker.exists())

    def test_legacy_launchers_contain_guidance_only(self):
        for script in self.scripts:
...
class AtomicWriteAndCliTests(unittest.TestCase):
    ...
    def test_build_generator_binds_location_and_radius_claims_to_source(self):
        ...
        adverse = (
            ...
            ("serving springfield and surrounding communities within 25 miles.", "service location"),
            ("We serve Springfield and surrounding communities.", "service location"),
            ("Test Business serves the Springfield area.", "service location"),
            ("we serve springfield and surrounding communities.", "service location"),
